...
# n_x, n_y, n_z = 4, 4, 4  # size of the unitcell
# n_ = 4
# sigma_ = 1
# a = np.power(2,(1./6))*sigma_
# b_1 = 1/np.sqrt(2)*np.array([0,1,1])*a                #%%%%%%%%%%%%%%%%%%%%%%
# b_2 = 1/np.sqrt(2)*np.array([1,0,1])*a   			  #% Bond displacements %
# b_3 = 1/np.sqrt(2)*np.array([1,1,0])*a		 		  #%%%%%%%%%%%%%%%%%%%%%%
# k_step = 2*np.pi/(a*n_)
# k_min= -np.pi/a
# k_max= +np.pi/a
# K_points =
...
def force_constants(G,T=20):
	...
	for i in range(Natoms):
		for j in range(Natoms):
			for a in range(3):
				for b in range(3):
					print("i = {} j = {} , a = {} , b = {}".format(i,j,a,b),G[i,a,j,b])
	if (np.conj(G.T)==G): #.all(): # check if G is hermitian
